...
Object.keys(codes).forEach(function (name) {
  ...
  fns['is' + fnName + 'Packet'] = function (packet) {
    return !!(packet && (typeof packet === 'undefined' ? 'undefined' : _typeof$1(packet)) === 'object' && packet.t === codes[name]);
  };
});
...
var index = _extends({ codes: codes }, fns);
...
var require$$1 = ( node$1 && node ) || node$1;
...
var require$$0$3 = ( src$1 && src ) || src$1;
...
var Socket = function () {
  ...
  createClass(Socket, [{
    ...
    value: function close() {
      ...
      this.connection.sendPacket(index.leavePacket(this.topic));
    }
...
var Connection = function (_Emitter) {
  ...
  createClass(Connection, [{
    ...
    value: function _handleMessage(packet) {
      ...
      if (index.isJoinAckPacket(packet)) {
        if (process.env.NODE_ENV !== 'production') {
      ...
      if (index.isJoinErrorPacket(packet)) {
        if (process.env.NODE_ENV !== 'production') {
      ...
      if (index.isLeaveAckPacket(packet)) {
        if (process.env.NODE_ENV !== 'production') {
      ...
      if (index.isLeaveErrorPacket(packet)) {
        if (process.env.NODE_ENV !== 'production') {
    ...
    value: function _handleOpen(packet) {
      ...
      this._pingTimer = setInterval(function () {
        _this7.sendPacket(index.pingPacket());
    ...
    value: function _sendSubscriptionPacket(topic) {
      ...
      this.sendPacket(index.joinPacket(topic));
    }
    ...
    value: function sendEvent(topic, event, data) {
      ...
      this.sendPacket(index.eventPacket(topic, event, data));
    }
